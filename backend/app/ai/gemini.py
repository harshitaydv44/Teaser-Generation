...
class GeminiProvider(AIProvider):
    ...
    def _upload_and_activate(self, client, api_key: str, request: AnalysisRequest):
        ...
        try:
            ...
            if not file_name:
                raise AIError(f"Gemini upload returned no file name: {data}")
            ...
            uploaded.uri = file_data.get("uri")
            uploaded.mime_type = file_data.get("mimeType", "video/mp4")
            uploaded.state = types.FileState.ACTIVE

            if not uploaded.uri:
                raise AIError(f"Gemini upload returned no file URI: {data}")

            logger.info("Gemini file %s uploaded successfully via REST", uploaded.name)
    ...
    def _analyze_with(self, api_key: str, request: AnalysisRequest) -> RawCandidateList:
        ...
        uploaded = self._upload_and_activate(client, api_key, request)
